...
class DatasetProcessor:
    """Videoların frame'lerini çıkartarak fake/real olarak ayıran sınıf"""
    
    def __init__(self, dataset_root, output_root, frame_skip=1, extract_faces=False):
        """
        Args:
            dataset_root: Dataset klasörünün yolu (DFD_manipulated_sequences, DFD_original sequences)
            output_root: Çıktı klasörünün yolu (data/)
            frame_skip: Kaç framede birini kaydetmek istediğimiz (1 = her frame, 2 = her 2. frame vb.)
            extract_faces: True ise sadece yüz bölgelerini çıkart (deepfake tespiti için optimize)
        """
        ...
        self.fake_dir = self.output_root / "fake"
        ...
        if self.extract_faces:
            try:
                # OpenCV'nin built-in face cascade kullan
                cascade_path = cv2.data.haarcascades + 'haarcascade_frontalface_default.xml'
                self.face_detector = cv2.CascadeClassifier(cascade_path)
                if not self.face_detector.empty():
                    print("✅ OpenCV Face Detector (Haar Cascade) yüklendi")
                else:
                    print("⚠️  Face Detector yüklenemedi")
                    self.extract_faces = False
            except Exception as e:
                print(f"⚠️  Face Detector yüklenirken hata: {e}")
                self.extract_faces = False
    ...
    def extract_frames_from_video(self, video_path, output_dir, video_name):
        ...
        mode_text = " (FACE EXTRACTION)" if self.extract_faces else ""
        print(f"  📹 {video_name}: {frame_count} frame çıkartılıyor (Tahmini: {estimated_time:.1f} saniye){mode_text}...")
        ...
        with tqdm(total=frame_count, desc=f"  Frame çıkartılıyor", leave=False) as pbar:
            while True:
                ...
                if frame_id % self.frame_skip == 0:
                    if self.extract_faces and self.face_detector:
                        # Yüz tespiti yap (OpenCV Haar Cascade)
                        ...
                        if len(faces) > 0:
                            ...
                            # Yüz bölgesini çıkart (margin ekle)
                            margin = 20
                            x1 = max(0, x - margin)
                            y1 = max(0, y - margin)
                            ...
                            if face_frame.size > 0:
                                ...
                                cv2.imwrite(str(frame_path), face_frame)
                                saved_frame_count += 1
                                faces_found += 1
                    else:
                        # Tüm frame'i kaydet
        ...
        if self.extract_faces:
            print(f"  ✅ {saved_frame_count} frame kaydedildi (Yüz bulundu: {faces_found}) : {video_output_dir}")
        else:
    ...
    def calculate_total_duration(self):
        """Tüm videoların toplam süresini hesapla (optimize versiyonu)"""
        total_seconds = 0
        video_extensions = {'.mp4', '.avi', '.mov', '.flv', '.mkv'}
        all_videos = []
        
        # Fake videoları bul
        fake_dataset = self.dataset_root / "DFD_manipulated_sequences" / "DFD_manipulated_sequences"
        if fake_dataset.exists():
            videos = [f for f in fake_dataset.glob('**/*') if f.suffix.lower() in video_extensions]
        ...
        print(f"   📹 Toplam {len(all_videos)} video bulundu, bilgi alınıyor...")
    ...
    def process_fake_videos(self):
        """DFD_manipulated_sequences klasöründeki videoları işle"""
        print("\n🎬 FAKE (Manipulated) videoları işleniyor...")
        
        fake_dataset = self.dataset_root / "DFD_manipulated_sequences" / "DFD_manipulated_sequences"
        
        if not fake_dataset.exists():
            print(f"❌ Fake dataset klasörü bulunamadı: {fake_dataset}")
            return
        
        video_extensions = {'.mp4', '.avi', '.mov', '.flv', '.mkv'}
        videos = [f for f in fake_dataset.glob('**/*') if f.suffix.lower() in video_extensions]
        
        print(f"📊 Toplam {len(videos)} fake video bulundu")
        
        for video_path in videos:
            relative_path = video_path.relative_to(fake_dataset)
            self.extract_frames_from_video(video_path, self.fake_dir, str(relative_path))
    ...
    def process_all(self):
        ...
        print("\n📊 Videoların toplam süresi hesaplanıyor...")
        ...
        print(f"⏰ Başlama Zamanı: {self.start_time.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def _print_statistics(self):
        ...
        fake_frames = sum(len(files) for _, _, files in os.walk(self.fake_dir))
        real_frames = sum(len(files) for _, _, files in os.walk(self.real_dir))
        
        print(f"\n📊 İstatistikler:")
        print(f"   ✅ Fake frame'ler: {fake_frames}")
        print(f"   ✅ Real frame'ler: {real_frames}")
        print(f"   ✅ Toplam frame'ler: {fake_frames + real_frames}")


def process_dataset(dataset_path, output_path, frame_skip=5, extract_faces=False):
    """
    Videoları frame'lere dönüştür ve fake/real olarak ayır
    ...
    Args:
        ...
        extract_faces: True ise sadece yüz bölgelerini çıkart
    """
    processor = DatasetProcessor(dataset_path, output_path, frame_skip=frame_skip, extract_faces=extract_faces)
...
if __name__ == "__main__":
    ...
    # frame_skip: 1 = tüm frameler, 5 = her 5. frame, 30 = her 30. frame vb.
    # 6 saat -> 1 saat için yaklaşık 30 gerekli
    # extract_faces: True = sadece yüz bölgelerini kaydet (deepfake tespiti için önerilen)
    process_dataset(DATASET_PATH, OUTPUT_PATH, frame_skip=24, extract_faces=False)
